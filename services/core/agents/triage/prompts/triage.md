...
Como usar `outcome` e `confidence` ao consultar a memória (filtre pelo que QUER, os campos são positivos):
- Ao buscar uma CAUSA (`section: cause`, ou incidentes para diagnosticar), filtre `outcome: diagnosed` — relatórios inconclusivos não têm causa a oferecer, são ruído nessa busca.
- Na EXPLORAÇÃO (primeiras buscas, mapeando o terreno), NÃO filtre por `confidence` — um diagnóstico de confiança média ou baixa ainda é uma pista ("da última vez suspeitei disto"). Você quer amplitude aqui.
- Ao ANCORAR uma decisão num precedente ("vou seguir o que a memória sugere como causa provável"), aí sim filtre `confidence: high` — não apoie sua conclusão num diagnóstico passado que o próprio relatório marcou como fraco.
